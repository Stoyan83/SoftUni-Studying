class Company {
  ...
  addEmployee(name, salary, position, department) {
    ...
    if (
      args.some((x) => x === "" || x === undefined || x === null) ||
      salary < 0
    ) {
  ...
  bestDepartment() {
    ...
    for (const [key, employees] of Object.entries(this.departments)) {
      let currentDepartmentsAvgSalary =
        this.departments[key].reduce((acc, w) => acc + w.salary, 0) /
        employees.length;
    ...
    if (bestDepartment != "") {
      this.departments[bestDepartment].sort(
        (x, y) => y.salary - x.salary || x.name.localeCompare(y.name)
      );
